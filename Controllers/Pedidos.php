...
class Pedidos extends Controllers{
	...
	public function Pedidos()
	{
		...
		$this->views->getView($this,"pedidos",$data,"admin");
	}
	...
	public function orden($idpedido){
		...
		$this->views->getView($this,"orden",$data, "admin");
	}
		...
		if($_SESSION['permisosMod']['r'] and $_SESSION['userData']['idrol'] != RCLIENTES){
			if($transaccion == ""){
				...
				if(empty($requestTransaccion)){
					...
					$htmlModal = getFile("Templates/Modals/modalReembolso",$requestTransaccion);
		...
		if($_SESSION['permisosMod']['u'] and $_SESSION['userData']['idrol'] != RCLIENTES){
			if($pedido == ""){
				...
				if(empty($requestPedido)){
					...
					$htmlModal = getFile("Templates/Modals/modalPedido",$requestPedido);
					$arrResponse = array("status" => true, "html" => $htmlModal);
				}
			}
			echo json_encode($arrResponse,JSON_UNESCAPED_UNICODE);
